#ifndef CCL_PROJECT_FORMAT_HPP
#define CCL_PROJECT_FORMAT_HPP

#include <array>
#include <ccl/const_string.hpp>
#include <ccl/format/core/string_splitter.hpp>
#include <ccl/format/dumpers/int.hpp>
#include <ccl/format/dumpers/location.hpp>
...
namespace ccl::fmt
{
    ...
    struct StaticFormatterFrame
    {
        ...
        CCL_DECL auto begin() const noexcept
        {
            return buffer.begin();
        }

        CCL_DECL auto end() const noexcept
        {
    ...
    public:
        CCL_DECL auto get() noexcept -> std::u8string &
        {
    ...
    private:
        ...
        template<size_t Index>
        CCL_DECL static auto isNotEmptyBlock() noexcept -> bool
        {
        ...
        {
            if CCL_RUNTIME_BRANCH {// G++-12 doesn't allow to append string at compile time
