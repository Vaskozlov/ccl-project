    ...
    format_context &ctx) -> format_context::iterator
{
    ...
    fmt::format_to(
        ctx.out(), "{} -> [", item_print_wrapper.idToStringConverter(item.getProductionType()));
    ...
    for (std::size_t i = 0; i != item.dottedRule.size(); ++i) {
        ...
        fmt::format_to(
            ctx.out(), "{} ", item_print_wrapper.idToStringConverter(item.dottedRule.at(i)));
    }
    ...
    return fmt::format_to(
        ctx.out(), ", {}]", item_print_wrapper.idToStringConverter(item.getLookAhead()));
}
